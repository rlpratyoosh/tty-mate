...
pub fn convert_error_to_message(error: GameError) -> String {
    match error {
...
pub fn parse_server_message(message: &str) -> Result<ServerMessage, GameError> {
...
pub fn parse_client_message(message: &str) -> Result<ClientMessage, GameError> {
    let tokens: Vec<&str> = message.split(":").collect();
    let Some(mode) = tokens.get(0) else {
        return Err(GameError::InvalidMessage);
    };
...
pub fn client_message_to_string(message: &ClientMessage) -> String {
    match message {
        ClientMessage::Move { from, to, piece_type } => {
...
pub fn server_message_to_string(message: ServerMessage) -> String {
    match message {
        ServerMessage::GameStart { game_id, color } => {
